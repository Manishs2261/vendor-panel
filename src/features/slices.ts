...
export const fetchAnalytics = createAsyncThunk(
  ...
  async (period: "7d" | "30d" | "90d" | "1y" = "30d", { rejectWithValue }) => {
    try {
      const { data } = await analyticsApi.get({ period });
      return data;
    } catch (err: any) {
...
export const fetchPayouts = createAsyncThunk(
  ...
  async (
    ...
    try {
      const { data } = await paymentApi.getPayouts(params);
      return data;
    } catch (err: any) {
      return rejectWithValue(
...
export const fetchCommission = createAsyncThunk(
  "payments/fetchCommission",
  async (_, { rejectWithValue }) => {
    try {
      const { data } = await paymentApi.getCommission();
      return data;
    } catch (err: any) {
      return rejectWithValue("Failed to fetch commission");
    }
  },
);
